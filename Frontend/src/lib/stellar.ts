...
// Distinguishes wallet-connectivity failures from generic app errors so error
// boundaries and hooks can offer wallet-specific recovery copy (e.g. "reconnect")
...
export class WalletManager {
  ...
  async signTransaction(transactionXDR: string): Promise<string> {
    ...
    try {
      ...
      throw new WalletConnectionError('Transaction signing not yet implemented')
    } catch (error) {
